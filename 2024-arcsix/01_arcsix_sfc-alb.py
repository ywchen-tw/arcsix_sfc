...
def plot_spiral_old(
        date,
        tmhr_range=[15.15, 15.54],
        wvl0=550.0
        ):

    tmhr_range=[14.5861, 14.8833]

    date_s     = date.strftime('%Y%m%d')

    tmhr_range_legs = [[14.5806, 14.6236], [14.6236, 14.6639], [14.6639, 14.7014], [14.7014, 14.7528], [14.7528, 14.7944], [14.7944, 14.8417], [14.8417, 14.8861]]
    ...
    logic = (data_hsk['tmhr']>tmhr_range[0]) & (data_hsk['tmhr']<tmhr_range[1]) & \
            (np.abs(data_hsk['ang_pit'])<=3.0) & (np.abs(data_hsk['ang_rol'])<=3.0)

    alt = data_hsk['alt'][logic]/1000.0
    #\----------------------------------------------------------------------------/#


    # read SSFR v2
    #/----------------------------------------------------------------------------\#
    fname_ssfr1_ra = '%s/%s-SSFR_%s_%s_RA.h5' % (_fdir_data_, _mission_.upper(), _platform_.upper(), date_s)
    data_ssfr1_v2 = ssfr.util.load_h5(fname_ssfr1_ra)
    index_wvl_ssfr = np.argmin(np.abs(wvl0-data_ssfr1_v2['zen/wvl']))
    f_dn = data_ssfr1_v2['zen/flux'][logic, index_wvl_ssfr]
    index_wvl_ssfr = np.argmin(np.abs(wvl0-data_ssfr1_v2['nad/wvl']))
    f_up = data_ssfr1_v2['nad/flux'][logic, index_wvl_ssfr]
    ...
    #/----------------------------------------------------------------------------\#
    # data_spns_v2 = ssfr.util.load_h5(_fnames_['%s_spns_v2' % date_s])
    # index_wvl_spns = np.argmin(np.abs(wvl0-data_spns_v2['tot/wvl']))
    # f_dn_tot = data_spns_v2['tot/flux'][logic, index_wvl_spns]
    # f_dn_dif = data_spns_v2['dif/flux'][logic, index_wvl_spns]
    #\----------------------------------------------------------------------------/#
    ...
    ax1.axvspan(-0.1, 0.0, color='r', lw=0.0, alpha=0.1)
    ax1.set_xlim((-0.1, 1.0))
    ax1.set_ylim((0.0, 6.0))
    ax1.set_xlabel('Flux [$\\mathrm{W m^{-2} nm^{-1}}$]')
    ax1.set_ylabel('Altitude [km]')
    ax1.set_title('Spiral on %s (%.1f nm)' % (date.strftime('%Y-%m-%d'), wvl0))
    ...
    patches_legend = [
                ...
                # mpatches.Patch(color='cyan'  , label='SPN-S $F_\\downarrow^{tot}$')
                ]
...
def PLOT_ALBEDO():
    ...
    f = h5py.File('data/frac_info_20140913.h5', 'r')
    frac = f['frac'][...]
    logic_frac = f['logic'][...]
    f.close()

    f = h5py.File('data/ssfr_info_02_20140913.h5', 'r')
    tmhr = f['tmhr'][...]
    logic_ssfr = f['logic'][...]
    f_up = f['f_up'][...]
    f_dn = f['f_dn'][...]
    ...
    f = h5py.File('data/alp_info_20140913.h5', 'r')
    alt  = f['alt'][...]
    f.close()

    f0 = h5py.File('Albedo_All.h5', 'w')


    albedo = f_up/f_dn

    logic = logic_frac & logic_ssfr

    slope = np.zeros(wvl.size)
    intercept = np.zeros(wvl.size)

    frac_all   = np.zeros((frac.size, wvl.size))
    albedo_all = albedo.copy()
    logic_all  = np.zeros((frac.size, wvl.size))

    for i in range(wvl.size):

        logic_new = logic & np.logical_not(np.isnan(frac)) & np.logical_not(np.isnan(albedo[:, i]))
        logic_all[:, i] = logic_new
        frac_all[:, i]  = frac
        if logic_new.sum() > 0:
            slope[i], intercept[i], r_value, p_value, std_err = stats.linregress(frac[logic], albedo[logic, i])
        else:
            slope[i], intercept[i] = np.nan, np.nan

    wvl_arise0    = wvl.copy()
    albedo_arise0 = slope*1.0 + intercept
    wvl_arise, slope_arise     = ABSORB_FILTER(wvl_arise0, slope)
    wvl_arise, intercept_arise = ABSORB_FILTER(wvl_arise0, intercept)
    f0['wvl_arise']       = wvl_arise
    f0['slope_arise']     = slope_arise
    f0['intercept_arise'] = intercept_arise

    f0['frac_all']   = frac_all
    f0['wvl_all']    = wvl_arise0
    f0['albedo_all'] = albedo_all
    f0['logic_all']  = logic_all
    f0['alt_all']    = alt
    ...
    data = np.loadtxt('/argus/home/chen/work/02_arise/10_poster/ice_only.txt', delimiter=',')
    ax1.plot(data[:, 0], data[:, 1], color='lime', label='ice only (Brandt and Warren 2005)', marker='o', markeredgecolor='none', zorder=1)
    f0['wvl_ice_only_2005'] = data[:, 0]
    f0['albedo_ice_only_2005'] = data[:, 1]
    data = np.loadtxt('/argus/home/chen/work/02_arise/10_poster/ice_thin_snow.txt', delimiter=',')
    ax1.plot(data[:, 0], data[:, 1], color='limegreen', label='ice+thin snow (Brandt and Warren 2005)', marker='o', markeredgecolor='none', zorder=2)
    f0['wvl_ice_thin_snow_2005'] = data[:, 0]
    f0['albedo_ice_thin_snow_2005'] = data[:, 1]
    data = np.loadtxt('/argus/home/chen/work/02_arise/10_poster/ice_deep_snow.txt', delimiter=',')
    ax1.plot(data[:, 0], data[:, 1], color='forestgreen', label='ice+deep snow (Brandt and Warren 2005)', marker='o', markeredgecolor='none', zorder=3)
    f0['wvl_ice_thick_snow_2005'] = data[:, 0]
    f0['albedo_ice_thick_snow_2005'] = data[:, 1]

    wvl_ref = np.arange(300.0, 2001.0, 50.0)

    # perovich et al 2002
    data = np.loadtxt('/argus/home/chen/work/02_arise/10_poster/albedo_perovich.csv', delimiter=',')
    wvl_per    = data[:, 0]
    albedo_per = data[:, 1]
    ax1.plot(wvl_per, albedo_per, color='royalblue', label='melting bare white ice (Perovich et al. 2002)', marker='o', markeredgecolor='none', zorder=0)
    f0['wvl_ice_melt_2002'] = data[:, 0]
    f0['albedo_ice_melt_2002'] = data[:, 1]

    jen_wvl = np.array([659.0, 858.0, 1240.0, 1640.0, 2130.0, 3740.0])
    a_dry   = np.array([90.0, 85.0, 45.0, 15.0, 15.0, 5.0])/100.0
    a_wet   = np.array([85.0, 75.0, 25.0,  5.0,  5.0, 5.0])/100.0
    ax1.plot(jen_wvl, a_dry, color='indianred', label="dry season (Kay and L'Ecuyer 2013)", marker='s', markeredgecolor='none', zorder=4)
    ax1.plot(jen_wvl, a_wet, color='orangered', label="wet season (Kay and L'Ecuyer 2013)", marker='s', markeredgecolor='none', zorder=5)
    f0['wvl_dry_season_2013'] = jen_wvl
    f0['wvl_wet_season_2013'] = jen_wvl
    f0['albedo_dry_season_2013'] = a_dry
    f0['albedo_wet_season_2013'] = a_wet


    data = np.genfromtxt('/argus/home/chen/work/02_arise/10_poster/20080415ELalbedo.dat', skip_header=1)
    wvl_arctas    = data[:, 0]
    albedo_arctas = data[:, 1]
    wvl_arctas, albedo_arctas = ABSORB_FILTER(wvl_arctas, albedo_arctas)
    ax1.plot(wvl_arctas, albedo_arctas, color='violet', label='ARCTAS SSFR (site EL)', marker='o', markeredgecolor='none', zorder=6)
    f0['wvl_arctas'] = wvl_arctas
    f0['albedo_arctas'] = albedo_arctas


    wvl_arise, albedo_arise = ABSORB_FILTER(wvl_arise0, albedo_arise0)
    # wvl_arise, std_arise    = ABSORB_FILTER(wvl_arise0, std_arise0)
    ax1.plot(wvl_arise, albedo_arise, color='k', label='ARISE SSFR (IF=100%)', marker='*', markeredgecolor='k', markersize=8, zorder=9, lw=2.5)
    ax1.plot(wvl_arise[wvl_arise<350.0], albedo_arise[wvl_arise<350.0], color='gray', marker='*', markeredgecolor='gray', markersize=8, zorder=9, lw=2.5)

    # ax1.fill_between(wvl_arise, albedo_arise*(1.0-std_arise), albedo_arise*(1.0+std_arise), linewidth=0.0, facecolor='k', alpha=0.7, zorder=8)
    # ax1.plot(wvl_arise[4], albedo_arise[4], markerfacecolor='k', marker='*', markeredgecolor='red', markersize=18, markeredgewidth=2.0, zorder=10)

    f0.close()

    ax1.set_xlim([300, 1800])
    ax1.xaxis.set_major_locator(FixedLocator(np.arange(300, 1801, 300)))
    ax1.xaxis.set_minor_locator(FixedLocator(np.arange(300, 1801, 100)))
    ax1.set_ylim([0, 1.0])
    ax1.set_xlabel('Wavelength [nm]')
    ax1.set_ylabel('Albedo')
    ax1.legend(fontsize=10, framealpha=0.1, loc='upper right')
    plt.savefig('albedo_all.png', transparent=False)
    plt.show()
    exit()
...
def figure_sfc_alb_square_spiral_one(
        date,
        tmhr_range=[15.15, 15.54],
        wvl0=550.0
        ):
    ...
    alt_norm = mpl.colors.Normalize(vmin=0.0, vmax=5.0)
    #\----------------------------------------------------------------------------/#

    # read SSFR
    #/----------------------------------------------------------------------------\#
    fname_ssfr1_ra = '%s/%s-SSFR_%s_%s_RA.h5' % (_fdir_data_, _mission_.upper(), _platform_.upper(), date_s)
    data_ssfr1_ra = er3t.util.load_h5(fname_ssfr1_ra)
    #\----------------------------------------------------------------------------/#

    wvl = data_ssfr1_ra['zen/wvl'].copy()
    ...
    for absorb_band in absorb_bands:
        wvl_L, wvl_R = absorb_band
        ax1.axvspan(wvl_L, wvl_R, color='red', lw=0.0, alpha=0.1, zorder=0)

    for tmhr_range in tmhr_range_legs:
        logic = (data_hsk['tmhr']>tmhr_range[0]) & (data_hsk['tmhr']<tmhr_range[1]) & \
                (np.abs(data_hsk['ang_pit'])<=2.0) & (np.abs(data_hsk['ang_rol'])<=2.0)

        f_dn = data_ssfr1_ra['zen/flux'][logic, :]
        f_up = data_ssfr1_ra['nad/flux'][logic, 1:]

        alb_all = f_up/f_dn
        alb_mean = np.nanmean(alb_all, axis=0)
        alb_std  = np.nanstd(alb_all, axis=0)

        color=alt_cmap(alt_norm(np.mean(alt[logic])))
        ax1.scatter(data_ssfr1_ra['zen/wvl'], alb_mean, alpha=0.3, color=color, s=3, lw=0.0, zorder=0)
        ax1.errorbar(wvl, alb_mean, yerr=alb_std, color=color, zorder=1)


    # arise albedo
    #/----------------------------------------------------------------------------\#
    f0 = h5py.File('Albedo_All.h5', 'r')
    ...
    f0.close()
    ax1.scatter(arise_wvl, arise_albedo, color='k', label="ARISE SSFR (2014-09-11)", marker='o', ec='none', s=8, zorder=0, alpha=0.6)
    #\----------------------------------------------------------------------------/#
    ...
    f0.close()
    ax1.plot(lya_wvl, lya_alb, color='orange', label="Fresh snow (Lyapustin et al. 2010)", marker='o', markeredgecolor='none', markersize=10, lw=0.5, zorder=4, alpha=0.8)
    #\----------------------------------------------------------------------------/#
    ...
    f0.close()
    ax1.plot(brandt_wvl, brandt_alb, color='lime', label="Ice+deep snow (Brandt et al. 2005)", marker='o', markeredgecolor='none', markersize=10, lw=0.5, zorder=4, alpha=0.8)
    #\----------------------------------------------------------------------------/#
    ...
    f0.close()
    ax1.plot(jen_wvl, a_dry, color='gray', label="Dry season (Kay and L'Ecuyer 2013)", marker='o', markeredgecolor='none', markersize=10, lw=0.5, zorder=4, alpha=0.8)
    ax1.plot(jen_wvl, a_wet, color='gray', label="Wet season (Kay and L'Ecuyer 2013)", marker='s', markeredgecolor='none', markersize=10, lw=0.5, zorder=5, alpha=0.8)
    #\----------------------------------------------------------------------------/#


    cs = ax1.scatter(-1, -1, c=1, cmap='jet', vmin=0.0, vmax=5.0)
    divider = make_axes_locatable(ax1)
    cax = divider.append_axes('right', '3%', pad='3%')
    cbar = fig.colorbar(cs, cax=cax)
    cbar.set_label('Altitude [km]', rotation=270, labelpad=22.0)
    ...
    ax1.set_xlabel('Wavelength [nm]')
    ax1.set_ylabel('Albedo')
    ax1.set_title('Square Spiral on %s' % (date.strftime('%Y-%m-%d')), y=1.02)
    ...
    ax1.legend(fontsize=16, framealpha=0.1, loc='upper right')

    plt.savefig('%s_spiral.png' % date_s, bbox_inches='tight')
    plt.show()
    #\----------------------------------------------------------------------------/#

def figure_sfc_alb_square_spiral_all():
    ...
    colors = mpl.colormaps['jet'](np.linspace(0.0, 1.0, 10))

    tmhr_ranges_spiral = {
            '20240528': [15.1597, 15.5417],
            '20240530': [13.3333, 13.5125],
            '20240531': [15.1514, 15.6056],
            '20240603': [13.4222, 13.5750],
            '20240605': [13.7736, 14.2458],
            # '20240606': [13.3361, 13.6833],
            '20240606': [17.0833, 17.1931],
            '20240607': [14.9000, 15.2847],
            '20240610': [11.4931, 11.8597],
            '20240611': [14.5667, 14.9403],
            '20240613': [13.0181, 13.4222],
            }

    comments = {
            '20240528': 'Clear-Sky',
            '20240530': 'Cloudy',
            '20240531': 'Clear-Sky [heating issue]',
            '20240603': 'Cloudy',
            '20240605': 'Clear-Sky',
            # '20240606': 'Cloudy',
            '20240606': 'Clear-Sky',
            '20240607': 'Cloudy',
            '20240610': 'Clear-Sky [ocean]',
            '20240611': 'Clear-Sky',
            '20240613': 'Cloudy',
            }
    ...
    for i, date in enumerate(dates):
        ...
        tmhr_range_spiral = tmhr_ranges_spiral[date_s]
        tmhr_range_legs = []
        logic_spiral = (data_hsk['tmhr']>tmhr_range_spiral[0]) & (data_hsk['tmhr']<tmhr_range_spiral[1]) & \
                       (np.abs(data_hsk['ang_pit'])<=2.0) & (np.abs(data_hsk['ang_rol'])<=2.0)
        tmhr_spiral = data_hsk['tmhr'][logic_spiral]
        alt_spiral  = alt[logic_spiral]
        kt19_spiral = kt19[logic_spiral]

        time_diff = np.abs(tmhr_spiral[:-1] - tmhr_spiral[1:]) * 3600.0

        indices_s = np.append(0, np.where((time_diff > 30.0))[0]+1)
        indices_e = np.append(np.where((time_diff > 30.0))[0], tmhr_spiral.size-1)
        tmhr_range_legs = zip(tmhr_spiral[indices_s], tmhr_spiral[indices_e])
        Nleg = indices_s.size

        color = colors[i, ...]
        if 'issue' in comments[date_s].lower():
            zorder = -20
            alpha = 0.2
        elif 'cloudy' in comments[date_s].lower():
            zorder = -10
            alpha = 0.2
        else:
            zorder = 0
            alpha = 1.0
            patches_legend.append(mpatches.Patch(color=color, alpha=alpha, label='SF#%d on %s (%s, ST=%.1f$^\\circ C$)' % (i+1, date.strftime('%B-%d').replace('-0', ' ').replace('-', ' '), comments[date_s], np.nanmean(kt19_spiral))))

        if alt_spiral[0] > alt_spiral[-1]:
            index_low = Nleg-1
        else:
            index_low = 0

        for j, tmhr_range in enumerate(tmhr_range_legs):
            logic = (data_hsk['tmhr']>tmhr_range[0]) & (data_hsk['tmhr']<tmhr_range[1]) & \
                    (np.abs(data_hsk['ang_pit'])<=2.0) & (np.abs(data_hsk['ang_rol'])<=2.0)
            ...
            # ax1.scatter(data_ssfr1_ra['zen/wvl'], alb_mean, alpha=0.1, color=color, s=3, lw=0.0, zorder=zorder)
            if j != index_low:
                pass
                # ax1.plot(wvl, alb_mean, color=color, alpha=0.1, zorder=zorder+1)
            else:
                ax1.errorbar(wvl, alb_mean, yerr=alb_std, color=color, alpha=alpha, zorder=zorder+1)
    ...
    ax1.set_xlabel('Wavelength [nm]')
    ax1.set_ylabel('Albedo')
    ax1.set_title('Spectral Surface Albedo from Square Spirals', y=1.02)
...
def figure_sfc_alb_low_leg():
    ...
    for i, date in enumerate(dates):
        ...
        if 'issue' in comments[date_s].lower():
            ...
            zorder = -20
            alpha = 0.0
        elif 'ocean' in comments[date_s].lower():
            zorder = -10
            alpha = 0.0
        else:
            zorder = 0
        ...
        for j, tmhr_range in enumerate(tmhr_range_legs):
            ...
            logic = (data_hsk['tmhr']>tmhr_range[0]) & (data_hsk['tmhr']<tmhr_range[1]) & \
                    (np.abs(data_hsk['ang_pit'])<=2.0) & (np.abs(data_hsk['ang_rol'])<=2.0) & \
                    (~np.isnan(data_ssfr1_ra['zen/flux'][:, np.argmin(np.abs(532.0-data_ssfr1_ra['zen/wvl']))])) & \
                    (~np.isnan(data_ssfr1_ra['nad/flux'][:, np.argmin(np.abs(532.0-data_ssfr1_ra['nad/wvl']))])) & \
                    ((data_ssfr1_ra['zen/flux'][:, np.argmin(np.abs(532.0-data_ssfr1_ra['zen/wvl']))])>0.0)

            if logic.sum() > 0:

                f_dn = data_ssfr1_ra['zen/flux'][logic, :]

                f_up_ = data_ssfr1_ra['nad/flux'][logic, :]
                f_up = np.zeros_like(f_dn)
                for ii_ in range(f_dn.shape[0]):
                    f_up[ii_, :] = np.interp(data_ssfr1_ra['zen/wvl'], data_ssfr1_ra['nad/wvl'], f_up_[ii_, :])
                ...
                if j != index_low:
                    ...
                    ax1.errorbar(wvl, alb_mean, yerr=alb_std, color=color, alpha=alpha, zorder=zorder+100)
    ...
    arise_albedo = arise_slope*1.0 + arise_intercept
    f0.close()
    ax1.scatter(arise_wvl, arise_albedo, color='k', label="ARISE SSFR (2014-09-11)", marker='o', ec='none', s=10, zorder=1000, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    lya_alb = f0['albedo_fresh_snow_2010'][...]
    f0.close()
    ax1.plot(lya_wvl, lya_alb, color='orange', label="Fresh snow (Lyapustin et al. 2010)", marker='o', markeredgecolor='none', markersize=10, lw=1.5, zorder=1004, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    brandt_alb = f0['albedo_ice_thick_snow_2005'][...]
    f0.close()
    ax1.plot(brandt_wvl, brandt_alb, color='purple', label="Ice+deep snow (Brandt et al. 2005)", marker='o', markeredgecolor='none', markersize=10, lw=1.5, zorder=1004, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    a_wet = f0['albedo_wet_season_2013'][...]
    f0.close()
    ax1.plot(jen_wvl, a_dry, color='gray', label="Dry season (Kay and L'Ecuyer 2013)", marker='o', markeredgecolor='none', markersize=10, lw=1.5, zorder=1004, alpha=1.0)
    ax1.plot(jen_wvl, a_wet, color='gray', label="Wet season (Kay and L'Ecuyer 2013)", marker='s', markeredgecolor='none', markersize=10, lw=1.5, zorder=1005, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    # patches_legend += handles
    legend1 = ax1.legend(handles=patches_legend, loc='upper right', fontsize=14, numpoints=1, scatterpoints=3, markerscale=1)
...
def figure_sfc_alb_square_spiral_arcsix1():
    ...
    for i, date in enumerate(dates):
        ...
        if 'issue' in comments[date_s].lower():
            ...
            zorder = 0
            alpha = 0.6
            patches_legend.append(mpatches.Patch(color=color, alpha=1.0, label='SF#%d on %s (ST=%.1f$^\\circ C$)' % (sf_number[date_s], date.strftime('%B-%d').replace('-0', ' ').replace('-', ' '), np.nanmean(kt19_spiral))))
        ...
        for j, tmhr_range in enumerate(tmhr_range_legs):
            ...
            if logic.sum() > 0:
                ...
                if j != index_low:
                    ...
                    # ax1.errorbar(wvl, alb_mean, yerr=alb_std, color=color, alpha=alpha, zorder=zorder+100)
                    # ax1.fill_between(wvl, alb_mean-alb_std, alb_mean+alb_std, fc=color, lw=0.0, alpha=alpha, zorder=zorder+100)
                    # ax1.plot(wvl, alb_mean, lw=1.0, color=color, alpha=alpha, zorder=zorder+100)
    ...
    arise_albedo = arise_slope*1.0 + arise_intercept
    f0.close()
    ax1.scatter(arise_wvl, arise_albedo, color='k', label="ARISE SSFR (2014-09-11)", marker='o', ec='none', s=10, zorder=1, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    lya_alb = f0['albedo_fresh_snow_2010'][...]
    f0.close()
    ax1.plot(lya_wvl, lya_alb, color='orange', label="Fresh snow (Lyapustin et al. 2010)", marker='o', markeredgecolor='none', markersize=10, lw=1.5, zorder=2, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    brandt_alb = f0['albedo_ice_thick_snow_2005'][...]
    f0.close()
    ax1.plot(brandt_wvl, brandt_alb, color='purple', label="Ice+deep snow (Brandt et al. 2005)", marker='o', markeredgecolor='none', markersize=10, lw=1.5, zorder=3, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    a_wet = f0['albedo_wet_season_2013'][...]
    f0.close()
    ax1.plot(jen_wvl, a_dry, color='gray', label="Dry season (Kay and L'Ecuyer 2013)", marker='o', markeredgecolor='none', markersize=10, lw=1.5, zorder=4, alpha=1.0)
    ax1.plot(jen_wvl, a_wet, color='gray', label="Wet season (Kay and L'Ecuyer 2013)", marker='s', markeredgecolor='none', markersize=10, lw=1.5, zorder=5, alpha=1.0)
    #\----------------------------------------------------------------------------/#
    ...
    ax1.set_xlabel('Wavelength [nm]')
    ax1.set_ylabel('Albedo')
    # ax1.set_title('Spectral Surface Albedo from Square Spirals (ARCSIX-1)', y=1.02)
...
def figure_sfc_alb_square_spiral_arcsix2():
    ...
    tmhr_ranges_spiral = {
            '20240729': [13.7347, 13.7708],
            '20240730': [14.1583, 14.1764],
            '20240802': [14.4014, 14.4278],
            }
    ...
    for i, date in enumerate(dates):
        ...
        data_hsk = er3t.util.load_h5(fname_hsk)
        alt = data_hsk['alt']/1000.0
        if 'ir_surf_temp' in data_hsk.keys():
            kt19 = data_hsk['ir_surf_temp']
        else:
            kt19 = None
        #\----------------------------------------------------------------------------/#
        ...
        if 'issue' in comments[date_s].lower():
            ...
            if kt19 is not None:
                ...
                patches_legend.append(mpatches.Patch(color=color, alpha=1.0, label='SF#%d on %s' % (sf_number[date_s], date.strftime('%B-%d').replace('-0', ' ').replace('-', ' '))))
        ...
        for j, tmhr_range in enumerate(tmhr_range_legs):
            ...
            if logic.sum() > 0:
                ...
                if j != index_low:
                    ...
                    ax1.fill_between(wvl, alb_min, alb_max, fc=color, lw=0.0, alpha=0.3, zorder=zorder+100)
                    ax1.errorbar(wvl, alb_mean, yerr=alb_std, color=color, alpha=0.4, zorder=zorder+100)
                    ax1.plot(wvl, alb_mean, lw=2.0, color=color, alpha=1.0, zorder=zorder+100)
    ...
    ax1.set_xlabel('Wavelength [nm]')
    ax1.set_ylabel('Albedo')
    ax1.set_title('Spectral Surface Albedo from Square Spirals (ARCSIX-2)', y=1.02)
...
def figure_sfc_alb_low_leg_arcsix():
    ...
    # use different colors for ARCSIX-1 and ARCSIX-2
    # /--------------------------------------------------------------------------\ #
    Narcsix1 = sum([1 for date in dates if date<datetime.datetime(2024, 7, 1)])
    Narcsix2 = sum([1 for date in dates if date>datetime.datetime(2024, 7, 1)])
    colors1 = mpl.colormaps['Blues'](np.linspace(0.0, 1.0, Narcsix1+2))[1:-1, :]
    colors2 = mpl.colormaps['Reds'](np.linspace(0.0, 1.0, Narcsix2+2))[1:-1, :]
    ...
    ax1.set_xlabel('Wavelength [nm]')
    ax1.set_ylabel('Albedo')
    # ax1.set_title('Spectral Surface Albedo from Low-Level Clear-Sky Leg (ARCSIX)', y=1.02)
    ...
    _metadata = {'Computer': os.uname()[1], 'Script': os.path.abspath(__file__), 'Function':sys._getframe().f_code.co_name, 'Date':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    fig.savefig('%s.png' % (_metadata['Function']), bbox_inches='tight', metadata=_metadata, dpi=1000, transparent=True)
    # plt.show()
    #\----------------------------------------------------------------------------/#

def main_old():

    # figure_sfc_alb_square_spiral_arcsix1()
    # figure_sfc_alb_square_spiral_arcsix2()

    # figure_sfc_alb_low_leg_arcsix()
    ...
    # tmhr_range = [12.6417, 13.8083]
    # exclude_tmhr_ranges = [
    #         [12.8861, 13.2111],
    #         ]
    # cdata_sfc_alb(date, tmhr_range, exclude_tmhr_ranges=exclude_tmhr_ranges)
...
def cdata_sfc_alb(
    ...
    if indices.size > 0:
        for i, index in enumerate(indices):
            ...
            f_dn  = data_ssfr1_ra['zen/flux'][index, :]

            f_up_ = data_ssfr1_ra['nad/flux'][index, :]
            f_up  = np.interp(data_ssfr1_ra['zen/wvl'], data_ssfr1_ra['nad/wvl'], f_up_)
    ...
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname = 'data_albedo_%s.h5' % date_s
...
if __name__ == '__main__':
    ...
    # create surface albedo data for 06-11
    #╭────────────────────────────────────────────────────────────────────────────╮#
