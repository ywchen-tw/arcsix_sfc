...
def figure_cop_wall_20240611(
    ...
    if True:
        ...
        for i, index_sat in enumerate(sat_select):
            ...
            ax1.text(data_hsk[vname_x][index0], ylim1[1]*0.9, text0, color=color, fontsize=16, alpha=1.0, va='top', ha='left', rotation=270, zorder=1)
            ...
            logic1 = logics_select[1] & (data_sat['sat/ctp'][:, index_sat]!=1.0) & (data_sat['sat/cot'][:, index_sat]<0.0)
            width = np.nanmin(np.abs((data_hsk[vname_x][logic1][1:])-(data_hsk[vname_x][logic1][:-1])))
            bottom = np.repeat(ylim2[1]*0.98-0.02*i*ylim2[1], logic1.sum())
...
def figure_cop_flt_trk_20240611(
    ...
    if True:
        ...
        #/----------------------------------------------------------------------------\#
        fname = '%s/%s/%s' % (_fdir_sat_data_, date_s, fnames[sat_select[1]])
        f = Dataset(fname, 'r')
        lon_s = f.groups['geolocation_data'].variables['longitude'][...].data
        lat_s = f.groups['geolocation_data'].variables['latitude'][...].data
...
def run_rtm_lrt_20240611(
        fname_alb='data_albedo_20240611.h5',
        name_tag='arcsix_20240611',
        job_tag='cloud',
        fdir_tmp='tmp-data',
        ):
    ...
    alb = data_alb['albedo_interp']
    sza = data_alb['sza']
    alt = data_alb['alt']
    wvl = data_alb['wvl']
    cot = data_alb['CLDPROP_L2_VIIRS_NOAA20.A2024163.1530.011.2024164115425.nc/cot']
    cer = data_alb['CLDPROP_L2_VIIRS_NOAA20.A2024163.1530.011.2024164115425.nc/cer']
    cth = data_alb['CLDPROP_L2_VIIRS_NOAA20.A2024163.1530.011.2024164115425.nc/cth']
    ctp = data_alb['CLDPROP_L2_VIIRS_NOAA20.A2024163.1530.011.2024164115425.nc/ctp']
    #╰────────────────────────────────────────────────────────────────────────────╯#

    indices_select = np.where(cot>=0.0)[0]
    ...
    fdir_tmp = '%s/%s/%s/%s' % (fdir_tmp, name_tag, _metadata_['Function'], job_tag)
    ...
    #╭────────────────────────────────────────────────────────────────────────────╮#
    lrt_cfg = er3t.rtm.lrt.get_lrt_cfg()
    lrt_cfg['atmosphere_file'] = '%s/data/atmmod/afglsw.dat' % os.environ['LIBRADTRAN_V2_DIR']
    lrt_cfg['number_of_streams'] = 8
    lrt_cfg['mol_abs_param'] = 'reptran'
    #╰────────────────────────────────────────────────────────────────────────────╯#
    ...
    Ndata = sza.size
    Nwvl = wvl.size
    f_dn = np.zeros((Ndata, Nwvl), dtype=np.float64)
    f_up = np.zeros((Ndata, Nwvl), dtype=np.float64)
    #╰────────────────────────────────────────────────────────────────────────────╯#

    # for i in tqdm(range(Ndata)):
    for i in tqdm(indices_select):
        ...
        if cot0 > 0.0:
            ...
            cld_cfg = er3t.rtm.lrt.get_cld_cfg()
            cld_cfg['cloud_file']  = '%s/i-%5.5d_cloud.txt' % (fdir_tmp, i)
            cld_cfg['cloud_optical_thickness'] = cot0
            cld_cfg['cloud_effective_radius']  = cer0
            cld_cfg['cloud_altitude'] = np.arange(cth0-cgt, cth0+0.1, 0.1)
        else:
            cld_cfg = None
        #╰────────────────────────────────────────────────────────────────────────────╯#
        ...
        for j, wvl0 in enumerate(wvl):

            alb0 = alb[i, j]
            if not (alb0>=0.0 or alb0<=1.0):
                alb0 = 0.0
            if alb0 < 0.0:
                alb0 = 0.0
            ...
            init_flx = er3t.rtm.lrt.lrt_init_mono_flx(
                    input_file  = '%s/i-%5.5d_j-%3.3d_inp_%4.4dnm_flx.txt' % (fdir_tmp, i, j, wvl0),
                    output_file = '%s/i-%5.5d_j-%3.3d_out_%4.4dnm_flx.txt' % (fdir_tmp, i, j, wvl0),
                    date        = datetime.datetime(2024, 6, 11),
                    surface_albedo     = alb0,
                    solar_zenith_angle = sza0,
                    wavelength         = wvl0,
                    output_altitude    = alt0,
                    lrt_cfg            = lrt_cfg,
                    cld_cfg            = cld_cfg,
                    )
        ...
        print('Running calculations for <%s> ...' % (_metadata_['Function']))
        er3t.rtm.lrt.lrt_run_mp(inits_flx, Ncpu=12)

        try:
            data_flx = er3t.rtm.lrt.lrt_read_uvspec_flx(inits_flx)

            f_dn[i, :] = np.squeeze(data_flx.f_down)
            f_up[i, :] = np.squeeze(data_flx.f_up)
        except Exception as error:
            print(error)
            f_dn[i, :] = np.nan
            f_up[i, :] = np.nan
    ...
    # save h5 file
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname = 'data_rtm_lrt.h5'
    h5f = h5py.File(fname, 'w')
    if isinstance(f_dn, np.ndarray):
        h5d_f_dn = h5f.create_dataset('f_dn', data=f_dn, compression='gzip', compression_opts=9, chunks=True)
        h5d_f_up = h5f.create_dataset('f_up', data=f_dn, compression='gzip', compression_opts=9, chunks=True)
    else:
        h5f['f_dn'] = f_dn
        h5f['f_up'] = f_up
    h5f.close()
...
def run_rtm_lrt_20240607_high(
        fname_alb='data_albedo_20240607_high.h5',
        name_tag='arcsix_20240607',
        job_tag='cloud',
        fdir_tmp='tmp-data',
        ):
    ...
    # read data
    #╭────────────────────────────────────────────────────────────────────────────╮#
    # vname_sat = 'CLDPROP_L2_MODIS_Aqua.A2024159.1520.011.2024160162455.nc'
    vname_sat = 'CLDPROP_L2_VIIRS_SNPP.A2024159.1624.011.2024160114802.nc'
    data_alb = er3t.util.load_h5(fname_alb)
    alb = data_alb['albedo_interp']
    sza = data_alb['sza']
    alt = data_alb['alt']
    wvl = data_alb['wvl']
    cot = data_alb['%s/cot' % vname_sat]
    cer = data_alb['%s/cer' % vname_sat]
    cth = data_alb['%s/cth' % vname_sat]
    ctp = data_alb['%s/ctp' % vname_sat]
    #╰────────────────────────────────────────────────────────────────────────────╯#
    ...
    # save h5 file
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname = 'data_rtm_lrt_20240607_high.h5'
    h5f = h5py.File(fname, 'w')
    if isinstance(f_dn, np.ndarray):
        h5d_f_dn = h5f.create_dataset('f_dn', data=f_dn, compression='gzip', compression_opts=9, chunks=True)
        h5d_f_up = h5f.create_dataset('f_up', data=f_up, compression='gzip', compression_opts=9, chunks=True)
    else:
        h5f['f_dn'] = f_dn
        h5f['f_up'] = f_up
    h5f.close()
...
def figure_flux_compare_20240607_low():
    ...
    # read RTM
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname_rtm = 'data_rtm_lrt_%s_low.h5' % date_s
    data_rtm = er3t.util.load_h5(fname_rtm)
    #╰────────────────────────────────────────────────────────────────────────────╯#

    # read albedo
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname_alb = 'data_albedo_%s_low.h5' % date_s
    data_alb = er3t.util.load_h5(fname_alb)
    indices = data_alb['indices']
    #╰────────────────────────────────────────────────────────────────────────────╯#

    wvl = data_ssfr['zen/wvl']

    wvl0 = 555.0
    index_wvl = np.argmin(np.abs(wvl-wvl0))

    # for i in range(data_rtm['f_dn'].shape[0]):
    #     print(i, data_rtm['f_dn'][i, index_wvl])
    # sys.exit()
    index_ = 1267
    index = indices[index_]
    ...
    if plot:
        ...
        #╭──────────────────────────────────────────────────────────────╮#
        ax1 = fig.add_subplot(111)
        # ax1.scatter(data_ssfr['zen/wvl'], data_ssfr['zen/flux'][index, :], s=6, c='k', lw=0.0)
        # ax1.scatter(data_ssfr['zen/wvl'], data_rtm['f_dn'][index_, :], s=6, c='r', lw=0.0)
        ax1.scatter(data_ssfr['lat'][indices], data_ssfr['zen/flux'][indices, index_wvl], s=6, c='k', lw=0.0)
        ax1.scatter(data_ssfr['lat'][indices], data_rtm['f_dn'][:, index_wvl], s=6, c='r', lw=0.0)
        # ax1.scatter(data_ssfr['lat'][indices], data_ssfr['nad/flux'][indices, index_wvl], s=6, c='k', lw=0.0)
        # ax1.scatter(data_ssfr['lat'][indices], data_rtm['f_up'][:, index_wvl], s=6, c='r', lw=0.0)
        # ax1.set_xlim((0, 1))
        ax1.set_ylim((0, 1.0))
...
def figure_flux_compare_20240607_high():
    ...
    # read RTM
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname_rtm = 'data_rtm_lrt_%s_high.h5' % date_s
    data_rtm = er3t.util.load_h5(fname_rtm)
    #╰────────────────────────────────────────────────────────────────────────────╯#

    # read albedo
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname_alb = 'data_albedo_%s_high.h5' % date_s
    data_alb = er3t.util.load_h5(fname_alb)
    indices = data_alb['indices']
    #╰────────────────────────────────────────────────────────────────────────────╯#
    ...
    # factors = data_ssfr['zen/flux'][indices, index_wvl]/data_rtm['f_dn'][:, index_wvl]
    factors = 1.142
    ...
    if plot:
        ...
        # ax1.scatter(data_ssfr['lat'][indices], data_ssfr['zen/flux'][indices, index_wvl], s=6, c='k', lw=0.0)
        # ax1.scatter(data_ssfr['lat'][indices], data_ssfr['zen/flux'][indices, index_wvl]/factors, s=6, c='g', lw=0.0)
        # ax1.scatter(data_ssfr['lat'][indices], data_rtm['f_dn'][:, index_wvl], s=6, c='r', lw=0.0)

        ax1.scatter(data_ssfr['lat'][indices], data_ssfr['sza'][indices], s=6, c='r', lw=0.0)

        # ax1.scatter(data_ssfr['lat'][indices], data_ssfr['nad/flux'][indices, index_wvl]/factors, s=6, c='k', lw=0.0)
        # ax1.scatter(data_ssfr['lat'][indices], data_rtm['f_up'][:, index_wvl], s=6, c='r', lw=0.0)
        # ax1.set_xlim((0, 1))
        # ax1.set_ylim((0, 1.0))
        ax1.set_xlabel('Wavelength [nm]')
...
def run_rtm_lrt_20240607(
    ...
    #╭────────────────────────────────────────────────────────────────────────────╮#
    date_s = date.strftime('%Y%m%d')
    name_tag='%s_%s_%s' % (job_tag, date_s, leg_tag)
    #╰────────────────────────────────────────────────────────────────────────────╯#

    # create tmp directory
    #╭────────────────────────────────────────────────────────────────────────────╮#
    _metadata_ = {'Computer': os.uname()[1], 'Script': os.path.abspath(__file__), 'Function':sys._getframe().f_code.co_name, 'Date':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    fdir_tmp = '%s/%s/%s' % (fdir_tmp, _metadata_['Function'], name_tag)
    if not os.path.exists(fdir_tmp):
        os.makedirs(fdir_tmp)
    # else:
    #     os.system('rm -rf %s' % fdir_tmp)
    ...
    if 'fake' in job_tag.lower():
        fname_alb='data/data_albedo_fake_%s_%s.h5' % (date_s, leg_tag)
        print(fname_alb)
    else:
        fname_alb='data/data_albedo_%s_%s.h5' % (date_s, leg_tag)
    ...
    for i in tqdm(indices_select):
        ...
        # cloud setup
        #╭────────────────────────────────────────────────────────────────────────────╮#
        if (cot0 > 0.0) and ('cloud' in job_tag.lower()):
            # cgt = 1.0
            # cth0 = cth[i]/1000.0
            # if cth0 < 2.0:
            #     cth0 = 2.0
    ...
    # save h5 file
    #╭────────────────────────────────────────────────────────────────────────────╮#
    fname = 'data/data_rtm_lrt_%s.h5' % (name_tag)
    h5f = h5py.File(fname, 'w')
    if isinstance(f_dn, np.ndarray):
        h5d_f_dn = h5f.create_dataset('f_dn', data=f_dn, compression='gzip', compression_opts=9, chunks=True)
        h5d_f_up = h5f.create_dataset('f_up', data=f_up, compression='gzip', compression_opts=9, chunks=True)
...
if __name__ == '__main__':
    ...
    # 2024-06-07 case (cloudy)
    # using cot/cer
    # fix clouds to 0.3-0.5 km (inferred by MARLi)
    #╭────────────────────────────────────────────────────────────────────────────╮#
    # date = datetime.datetime(2024, 6, 7)
    # run_rtm_lrt_20240607(date, leg_tag='low', job_tag='cloud_fake')
    date = datetime.datetime(2024, 6, 7)
    run_rtm_lrt_20240607(date, leg_tag='high', job_tag='cloud_fake')
    #╰────────────────────────────────────────────────────────────────────────────╯#

    # 2024-06-07 case (clear)
    # using cot/cer
    # fix clouds to 0.3-0.5 km (inferred by MARLi)
    #╭────────────────────────────────────────────────────────────────────────────╮#
    # date = datetime.datetime(2024, 6, 7)
    # run_rtm_lrt_20240607(date, leg_tag='low', job_tag='clear_fake')
    date = datetime.datetime(2024, 6, 7)
    run_rtm_lrt_20240607(date, leg_tag='high', job_tag='clear_fake')
    #╰────────────────────────────────────────────────────────────────────────────╯#

    # figure_cop_wall_20240607()
    # figure_flux_compare_20240607_low()
    # figure_flux_compare_20240607_high()
