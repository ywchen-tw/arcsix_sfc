...
if __name__ == "__main__":
    ...
    hsk_data_all = {}
    hsk_data_selected = {}
    hsk_data_selected_clear = {}
    hsk_data_selected_cloudy = {}
    ...
    for hsk_filename in hsk_files:
        ...
        if date_str_formatted in clear_cases: # specially selected dates/cases
            hsk_data_item = {}
            for time_range in clear_cases[date_str_formatted]:
                start_time, end_time = time_range
                tmhr = hsk_data['hrs']
                start_hour, start_minute, start_second = map(int, start_time.split(':'))
                end_hour, end_minute, end_second = map(int, end_time.split(':'))
                ...
                selected_data = {k: v[time_mask] for k, v in hsk_data.items()}
                hsk_data_item[(start_time, end_time)] = selected_data
            hsk_data_selected_clear[date_str_formatted] = hsk_data_item
        if date_str_formatted in cloudy_cases: # specially selected dates/cases
            hsk_data_item = {}
            for time_range in cloudy_cases[date_str_formatted]:
                start_time, end_time = time_range
                tmhr = hsk_data['hrs']
                start_hour, start_minute, start_second = map(int, start_time.split(':'))
                end_hour, end_minute, end_second = map(int, end_time.split(':'))
                ...
                selected_data = {k: v[time_mask] for k, v in hsk_data.items()}
                hsk_data_item[(start_time, end_time)] = selected_data
            hsk_data_selected_cloudy[date_str_formatted] = hsk_data_item
    ...
    ax.set_extent([-85, -20, 76, 86], crs=ccrs.PlateCarree())
    ...
    clear_label = True
    cloudy_label = True
    for idate, (date_str, data) in enumerate(sorted(hsk_data_selected_clear.items())):
        for itime, (time_range, selected_data) in enumerate(data.items()):
            ...
            if clear_label:
                ax.plot(
                    ...
                    label="Clear sky cases"
                )
                clear_label = False
            else:
                ax.plot(lon, lat, color='tab:blue', linewidth=1.25, transform=ccrs.PlateCarree())
    for idate, (date_str, data) in enumerate(sorted(hsk_data_selected_cloudy.items())):
        for itime, (time_range, selected_data) in enumerate(data.items()):
            lon = selected_data['lon'][...]#[::5]
            lat = selected_data['lat'][...]#[::5]
            all_sel_lons.extend(lon)
            ...
            if cloudy_label:
                ...
                ax.plot(lon, lat, color='tab:red', linewidth=1.25, transform=ccrs.PlateCarree())
    ...
    #     print("Min/Max Lons:", min_lon, max_lon)
    #     print("Min/Max Lats:", min_lat, max_lat)
    #     lon_margin = (max_lon - min_lon) * 0.0025
    #     lat_margin = (max_lat - min_lat) * 0.0025
    #     min_lon -= lon_margin
    #     max_lon += (lon_margin - 1)
    #     min_lat -= (lat_margin + 0.5)
    #     max_lat += (lat_margin - 0.2)
    #     print("New Min/Max Lons:", min_lon, max_lon)
    #     print("New Min/Max Lats:", min_lat, max_lat)
    ...
    #     axins.add_feature(cfeature.LAND.with_scale('10m'), facecolor='white')
    #     axins.add_feature(cfeature.OCEAN.with_scale('10m'), facecolor=ocean_color)
    ...
    #             lon = selected_data['lon'][...][::5]
    #             lat = selected_data['lat'][...][::5]
    #             axins.plot(lon, lat, color='tab:blue', linewidth=1.25, transform=ccrs.PlateCarree())
    #     for idate, (date_str, data) in enumerate(sorted(hsk_data_selected_cloudy.items())):
    #         for itime, (time_range, selected_data) in enumerate(data.items()):
    #             lon = selected_data['lon'][...][::5]
    #             lat = selected_data['lat'][...][::5]
    #             axins.plot(lon, lat, color='tab:red', linewidth=1.25, transform=ccrs.PlateCarree())
