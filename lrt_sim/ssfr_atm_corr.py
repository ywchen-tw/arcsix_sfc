...
def atm_corr_plot(date=datetime.datetime(2024, 5, 31),
    ...
    gas_mask = np.isfinite(gas_abs_masking(alb_wvl, np.ones_like(alb_wvl, dtype=float)))
...
def flt_trk_atm_corr(date=datetime.datetime(2024, 5, 31),
    ...
    for ileg, _ in enumerate(leg_masks):
        ...
        if not os.path.exists(fname_h5) or overwrite_lrt: 
            ...
            print("alb_obs shape:", alb_obs.shape)
            print("corr_dn shape:", corr_dn.shape)
...
if __name__ == '__main__':
    ...
    for iter in range(3):
        flt_trk_atm_corr(date=datetime.datetime(2024, 6, 11),
                        tmhr_ranges_select=[
                                            ...
                                            # [14.968, 15.115], # 100-450m, clear, some cloud
                                            ],
                        case_tag='clear_atm_corr',
                        config=config,
    ...
    for iter in range(3):
        flt_trk_atm_corr(date=datetime.datetime(2024, 8, 2),
                        tmhr_ranges_select=[[14.557, 15.100], # 100m
                                            [15.244, 16.635], # 1km
                                            ],
                        case_tag='clear_atm_corr',
                        config=config,
