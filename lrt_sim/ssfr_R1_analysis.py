...
def flt_trk_atm_corr(date=datetime.datetime(2024, 5, 31),
    ...
    flux_solar_raw_interp = solar_interpolation_func(solar_flux_file='arcsix_ssfr_solar_flux_raw.dat', date=date)
    ...
    for i, mask in enumerate(leg_masks):
        ...
        ssfr_zen_toa_raw = flux_solar_raw_interp(data_ssfr['wvl_dn']) * np.cos(np.deg2rad(sza_hsk))[:, np.newaxis]  # W/m^2/nm
        ssfr_zen_wvl = data_ssfr['wvl_dn']
        ssfr_nad_wvl = data_ssfr['wvl_up']
        ...
        # ssfr_zen_toa_raw[~pitch_roll_mask, :] = np.nan
        ...
        pitch_roll_exceed = (data_ssfr['flag'] & ssfr_flags.pitcth_roll_exceed_threshold) != 0
        alp_ang_pit_rol_issue = (data_ssfr['flag'] & ssfr_flags.alp_ang_pit_rol_issue) != 0
        zen_toa_over_issue = (data_ssfr['flag'] & ssfr_flags.zen_toa_over_threshold) != 0
        ...
        leg['ssfr_toa_raw'] = ssfr_zen_toa_raw
        
        leg['ssfr_icing'] = icing[sel_ssfr]
        leg['ssfr_icing_pre'] = icing_pre[sel_ssfr]
        leg['ssfr_pitch_roll_exceed'] = pitch_roll_exceed[sel_ssfr]
        leg['ssfr_alp_ang_pit_rol_issue'] = alp_ang_pit_rol_issue[sel_ssfr]
        leg['ssfr_zen_toa_over_issue'] = zen_toa_over_issue[sel_ssfr]
    ...
    for ileg, _ in enumerate(leg_masks):
        ...
        ssfr_icing = cld_leg['ssfr_icing']
        ssfr_icing_pre = cld_leg['ssfr_icing_pre']
        ssfr_pitch_roll_exceed = cld_leg['ssfr_pitch_roll_exceed']
        ssfr_alp_ang_pit_rol_issue = cld_leg['ssfr_alp_ang_pit_rol_issue']
        ssfr_zen_toa_over_issue = cld_leg['ssfr_zen_toa_over_issue']
        
        plt.figure(figsize=(8, 6))
        plt.plot(ssfr_icing, 'r-', label='SSFR Icing')
        plt.plot(ssfr_icing_pre, 'b-', label='SSFR Icing Pre')
        plt.plot(ssfr_pitch_roll_exceed, 'g-', label='SSFR Pitch/Roll Exceed')
        plt.plot(ssfr_alp_ang_pit_rol_issue, 'm-', label='SSFR ALP Pitch/Roll Issue')
        plt.plot(ssfr_zen_toa_over_issue, 'k-', label='SSFR Zen TOA Over Issue')
        plt.xlabel('Index')
        plt.ylabel('Flag Status')
        plt.title(f'SSFR Flag Status for Leg {ileg+1} Time {time_start:.2f}-{time_end:.2f}h')
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.5)
        plt.savefig('fig/%s/%s_%s_time_%.2f-%.2f_ssfr_flag_status.png' % (date_s, date_s, case_tag, time_start, time_end), bbox_inches='tight', dpi=150)
        plt.close()
        ...
        fup_mean_no_pitchroll_exceed = np.nanmean(cld_leg['ssfr_nad'][~ssfr_pitch_roll_exceed, :], axis=0)
        fdn_mean_no_pitchroll_exceed = np.nanmean(cld_leg['ssfr_zen'][~ssfr_pitch_roll_exceed, :], axis=0)
        
        fup_mean_no_alp_pitrol_issue = np.nanmean(cld_leg['ssfr_nad'][~ssfr_alp_ang_pit_rol_issue, :], axis=0)
        fdn_mean_no_alp_pitrol_issue = np.nanmean(cld_leg['ssfr_zen'][~ssfr_alp_ang_pit_rol_issue, :], axis=0)
        
        fup_meam_no_icing = np.nanmean(cld_leg['ssfr_nad'][~ssfr_icing, :], axis=0)
        fdn_mean_no_icing = np.nanmean(cld_leg['ssfr_zen'][~ssfr_icing, :], axis=0)
        
        fup_mean_no_zen_toa_over = np.nanmean(cld_leg['ssfr_nad'][~ssfr_zen_toa_over_issue, :], axis=0)
        fdn_mean_no_zen_toa_over = np.nanmean(cld_leg['ssfr_zen'][~ssfr_zen_toa_over_issue, :], axis=0)
            
        

        fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(18, 4.5))
        l1 = ax1.plot(cld_leg['ssfr_zen_wvl'], fdn_mean, linewidth=1.5, color='k', label='all')
        ax1.plot(cld_leg['ssfr_zen_wvl'], fdn_mean_no_pitchroll_exceed, color='orange', linewidth=1.0, label='no pitch/roll exceed')
        l2 = ax1.plot(cld_leg['ssfr_zen_wvl'], fdn_mean_no_alp_pitrol_issue, color='red', linewidth=1.0, label='no ALP pitch/roll issue')
        l3 = ax1.plot(cld_leg['ssfr_zen_wvl'], fdn_mean_no_zen_toa_over, color='green', linewidth=1.0, label='no zen TOA over')
        l4 = ax1.plot(cld_leg['ssfr_zen_wvl'], fdn_mean_no_icing, color='blue', linewidth=1.0, label='no icing issue')
        ax1.plot(cld_leg['ssfr_zen_wvl'], np.nanmean(cld_leg['ssfr_toa'], axis=0), color='yellow', linestyle='--', linewidth=2.5, label='SSFR TOA (raw)')
        
        ax2.plot(cld_leg['ssfr_zen_wvl'], fup_mean, linewidth=1.5, color='k', label='all')
        ax2.plot(cld_leg['ssfr_zen_wvl'], fup_mean_no_pitchroll_exceed, color='orange', linewidth=1.0, label='no pitch/roll exceed')
        ax2.plot(cld_leg['ssfr_zen_wvl'], fup_mean_no_alp_pitrol_issue, color='red', linewidth=1.0, label='no ALP pitch/roll issue')
        ax2.plot(cld_leg['ssfr_zen_wvl'], fup_mean_no_zen_toa_over, color='green', linewidth=1.0, label='no zen TOA over')
        ax2.plot(cld_leg['ssfr_zen_wvl'], fup_meam_no_icing, color='blue', linewidth=1.0, label='no icing issue')
        
        ax3.plot(cld_leg['ssfr_zen_wvl'], fup_mean/fdn_mean, linewidth=1.5, color='k', label='all')
        ax3.plot(cld_leg['ssfr_zen_wvl'], fup_mean_no_pitchroll_exceed/fdn_mean_no_pitchroll_exceed, color='orange', linewidth=1.0, label='no pitch/roll exceed')
        ax3.plot(cld_leg['ssfr_zen_wvl'], fup_mean_no_alp_pitrol_issue/fdn_mean_no_alp_pitrol_issue, color='red', linewidth=1.0, label='no ALP pitch/roll issue')
        ax3.plot(cld_leg['ssfr_zen_wvl'], fup_mean_no_zen_toa_over/fdn_mean_no_zen_toa_over, color='green', linewidth=1.0, label='no zen TOA over')
        ax3.plot(cld_leg['ssfr_zen_wvl'], fup_meam_no_icing/fdn_mean_no_icing, color='blue', linewidth=1.0, label='no icing issue')
        ...
        ax1.set_ylabel('SSFR Downward flux', fontsize=12)
        ax2.set_ylabel('SSFR Upward flux', fontsize=12)
        ax3.set_ylabel('SSFR Up/Down flux ratio', fontsize=12)
        
        ax1.set_title('SSFR Downward flux', fontsize=14)
        ax2.set_title('SSFR Upward flux', fontsize=14)
        ax3.set_title('SSFR Up/Down flux ratio', fontsize=14)
        
                
        
        ax3.legend(loc='center left', bbox_to_anchor=(1.10, 0.5), fontsize=10)
        
        fig.suptitle(f'{date_s} {time_start:.3f}-{time_end:.3f} Alt {alt_avg:.2f}km', fontsize=10)
        fig.tight_layout()
        fig.savefig('fig/%s/%s_%s_time_%.2f-%.2f_alt-%.2fkm_updnflux_analysis.png' % (date_s, date_s, case_tag, time_start, time_end, alt_avg), bbox_inches='tight', dpi=450)
...
if __name__ == '__main__':
    ...
    flt_trk_atm_corr(date=datetime.datetime(2024, 8, 8),
                    tmhr_ranges_select=[
                                        [13.0, 14.55], # 180m, clear
                                        ],
                    case_tag='clear_anal_1',
                    config=config,
                    simulation_interval=None,
                    clear_sky=True,
                    overwrite_lrt=atm_corr_overwrite_lrt,
