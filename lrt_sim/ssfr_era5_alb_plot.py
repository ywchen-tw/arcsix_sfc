...
def combined_atm_corr():
    ...
    era5_time_dates = np.array([datetime.datetime(2024,5,1) + datetime.timedelta(days=t) for t in era5_time])
    ...
    for date_s in sorted(set(dates_spring_all)):
        ...
        if date_mask.sum() > 0:
            ...
            era5_alb_mesh = griddata(
                        (era5_lon.flatten(), era5_lat.flatten()), era5_alb_date[0].flatten(),
                        (lon_all_spring[date_mask], lat_all_spring[date_mask]),
                        method='nearest'
                    )
            ...
            lat_max = np.max(lat_all) + 2
            fig, ax = plt.subplots(figsize=(8,6), subplot_kw={'projection': ccrs.NorthPolarStereo(central_longitude=central_lon)})
            c1 = ax.pcolormesh(lon.reshape(lonlat_shape), lat.reshape(lonlat_shape), era5_alb_date[0].reshape(lonlat_shape), transform=ccrs.PlateCarree(), cmap='YlGn', vmin=0, vmax=1)
            c2 = ax.scatter(lon_all_spring[date_mask], lat_all_spring[date_mask], c=era5_alb_spring_all[date_mask], transform=ccrs.PlateCarree(), cmap='YlGn', vmin=0, vmax=1, edgecolors='k')
            ax.coastlines()
            ax.add_feature(cfeature.LAND, zorder=0, edgecolor='black', facecolor='lightgray')
            ax.set_extent([lon_min, lon_max, lat_min, lat_max], crs=ccrs.PlateCarree())
    ...
    for date_s in sorted(set(dates_summer_all)):
        ...
        if date_mask.sum() > 0:
            ...
            fig, ax = plt.subplots(figsize=(8,6), subplot_kw={'projection': ccrs.NorthPolarStereo(central_longitude=central_lon)})
            c1 = ax.pcolormesh(lon.reshape(lonlat_shape), lat.reshape(lonlat_shape), era5_alb_date[0].reshape(lonlat_shape), transform=ccrs.PlateCarree(), cmap='YlGn', vmin=0, vmax=1)
            c2 = ax.scatter(lon_all_summer[date_mask], lat_all_summer[date_mask], c=era5_alb_summer_all[date_mask], transform=ccrs.PlateCarree(), cmap='YlGn', vmin=0, vmax=1, edgecolors='k')
            ax.coastlines()
            ax.add_feature(cfeature.LAND, zorder=0, edgecolor='black', facecolor='lightgray')
            ax.set_extent([lon_min, lon_max, lat_min, lat_max], crs=ccrs.PlateCarree())
            cb = fig.colorbar(c1, ax=ax, label='ERA5 Forecast Albedo')
